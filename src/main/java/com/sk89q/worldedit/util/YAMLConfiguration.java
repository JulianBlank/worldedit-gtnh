...
public class YAMLConfiguration extends LocalConfiguration {
    ...
    public void load() {
        ...
        maxChangeLimit = Math.max(-1, config.getInt("limits.max-blocks-changed.maximum", maxChangeLimit));

        defaultMaxPolygonalPoints = Math
            .max(-1, config.getInt("limits.max-polygonal-points.default", defaultMaxPolygonalPoints));
        maxPolygonalPoints = Math.max(-1, config.getInt("limits.max-polygonal-points.maximum", maxPolygonalPoints));

        defaultMaxPolyhedronPoints = Math
            .max(-1, config.getInt("limits.max-polyhedron-points.default", defaultMaxPolyhedronPoints));
        ...
        maxSuperPickaxeSize = Math.max(1, config.getInt("limits.max-super-pickaxe-size", maxSuperPickaxeSize));
        ...
        spreadGroundBlocks = new HashSet<Integer>(config.getIntList("limits.spread-ground-blocks", null));
        ...
        superPickaxeDrop = config.getBoolean("super-pickaxe.drop-items", superPickaxeDrop);
        superPickaxeManyDrop = config.getBoolean("super-pickaxe.many-drop-items", superPickaxeManyDrop);
        ...
        useInventoryOverride = config.getBoolean("use-inventory.allow-override", useInventoryOverride);
        useInventoryCreativeOverride = config
            .getBoolean("use-inventory.creative-mode-overrides", useInventoryCreativeOverride);
        ...
        String type = config.getString("shell-save-type", "")
            .trim();
